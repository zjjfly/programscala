...
object Fold extends App {
  ...
  assert(List(List(1, 3, 4), List(2, 4, 5)).aggregate(0)((b, list) => b + list.sum, _ + _) == 19) //aggregate用的不多，因为它的参数多而且不容易理解
  ...
  assert(list2.reduceLeft((x, y) => s"($x)-($y)") != list2.reduceRight((x, y) => s"($x)-($y)"))
