...
object Filter extends App {
  ...
  assert(
    stateCapitals.filterNot { case (k, _) => k.startsWith("A") } == Map(
      "Wyoming" -> "Cheyenne"))
  assert(
    stateCapitals
      .find(kv => kv._1.equals("Alabama"))
      .contains("Alabama" -> "Montgomery"))
  assert(stateCapitals.forall(kv => kv._1.length > 3))
  assert(
    stateCapitals.partition(kv => kv._1.startsWith("A")) == (Map(
      "Alabama" -> "Montgomery",
      "Alaska" -> "Juneau"), Map("Wyoming" -> "Cheyenne")))
  assert(stateCapitals.take(1) == Map("Alabama" -> "Montgomery"))
  assert(
    stateCapitals.takeWhile(kv => kv._1.startsWith("A")) == Map(
      "Alabama" -> "Montgomery",
      "Alaska" -> "Juneau"))
}
