...
import Chapter1.Messages.{Exit, Finished, Response}
...
object ShapesActorDriver{
  def main(args: Array[String]) {
    ...
    val driver=system.actorOf(Props(new ShapesActorDriver(drawer)),"drawingService")
    driver ! Start
  }
}
class ShapesActorDriver(drawActor:ActorRef) extends Actor{
  override def receive: Receive = {
    ...
    case unexpected=>
      println("ShapesActorDriver: ERROR:Received an unexpected message = "+unexpected)
  }
}
